    ...
    class Program
    {
        ...
        static void Main(string[] args)
        {
            Task.Run(async () =>
            {
                ...
                try
                {
                    await DownloadSubtitle(movieName, lucky, languages);
        ...
        static async Task DownloadSubtitle(string movieFileName, bool lucky, IList<string> languages)
        {
            ...
            using (var osdb = await new Osdb().Login(systemLanguage, "OS Test User Agent"))
            {
                ...
                string subtitleFile =
                    await osdb.DownloadSubtitleToPath(Path.GetDirectoryName(movieFileName), selectedSubtitle);
        ...
        private static bool PromptForSubtitle(IList<Subtitle> subtitles, ref Subtitle selectedSubtitle)
        {
            ...
            do
            {
                Console.Write("Your choice? (avaible options: '1'-'{0}', 'c', 'l')  [{1}]:", subtitles.Count,
                ...
                if (!validOption)
                {
                    ...
                    {
                        Console.WriteLine(
                            "Invalid option. Please chose one of the index numbers from the list, 'c' to cancel or 'l' to show the list again.");
                    }
                }
            } while (!validOption);
        ...
        private static void DisplayChoices(IList<Subtitle> subtitles, Subtitle defaultSubtitle)
        {
            ...
            {
                Console.WriteLine("{0}[{1}] {2}", subtitle.Equals(defaultSubtitle) ? "*" : " ", choice++,
                    subtitle.MovieName);
